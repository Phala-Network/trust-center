...
export async function getPhalaCloudInfo(
  ...
  if (!response.ok) {
    if (response.status === 500) {
      throw new Error(
        `App '${contractAddress}' not found or is currently down on Phala Cloud`,
      )
    }
    throw new Error(
      `Failed to fetch system info from Phala Cloud: ${response.status} ${response.statusText}`,
    )
  }
...
export async function getRedpillInfo(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch Redpill info: ${response.status} ${response.statusText}`,
    )
  }
